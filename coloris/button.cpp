...
namespace nana{	namespace drawerbase
{
	namespace button
	{
		...
		void trigger::_m_draw_title(graph_reference graph, bool enabled)
		{
			...
			if(ts.width)
			{
				...
				if(enabled)
				{
					...
					auto text_color = attr_.fgcolor; //(attr_.focus_color && attr_.focused ? ::nana::color(colors::blue) : attr_.fgcolor);
		...
		void trigger::_m_draw_background(graph_reference graph)
		{
			...
			graph.rectangle(r, true, attr_.bgcolor);
		}
		...
		void trigger::_m_draw_border(graph_reference graph)
		{
			...
			nana::color goldenrod = nana::color(218, 165, 32);

			if (attr_.e_state == element_state::hovered or attr_.e_state == element_state::focus_hovered)
				graph.frame_rectangle(r, goldenrod, goldenrod, goldenrod, goldenrod);
			else
				graph.frame_rectangle(r, lt, lt, rb, rb);
				
			graph.palette(false, color(37, 37, 37)); //Alter this color for those pesky 4 pixels...
