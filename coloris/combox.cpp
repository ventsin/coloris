		...
		namespace combox
		{
				...
				void editable(bool enb)
				{
					if(editor_)
					{
						...
						if (!enb)
						{
							...
							{
								auto clr_from = colors::black;//colors::button_face_shadow_start;
								...
								graph.rectangle(::nana::rectangle(graph.size()).pare_off(pare_off_px), true, nana::color(20, 20, 20));
			...
			private:
				...
				void _m_draw_push_button(bool enabled)
				{
					...
					button.draw(*graph_, ::nana::color(198, 145, 12), colors::white, r, estate);
