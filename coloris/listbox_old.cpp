		...
		namespace listbox
		{
			...
			struct essence
			{
				...
				unsigned item_height() const noexcept
				{
					...
					content_view->enable_scrolls(widgets::skeletons::content_view::scrolls::none);
					return px;
				}
			...
			private:
				...
				)
				{
					...
					rectangle bground_r{ -5 + content_r.x + static_cast<int>(essence_->header.margin()), coord.y, show_w + 5, essence_->item_height() };
							...
							if (0 == display_order)
							{
								...
								if (essence_->if_image)
								{
									...
									if (item.img)
									{
										...
										img_r = imgt;
										img_r.x = 8 + content_pos + coord.x + (16 - static_cast<int>(item.img_show_size.width)) / 2;  // center in 16 - geom scheme?
