	...
	namespace drawerbase
	{
		...
		//private:
			void drawer::_m_background(graph_reference graph)
			{
				...
				auto result = r.result();
				if (!result.empty())
					graph.rectangle(result, true, static_cast<color_rgb>(62, 62, 62));
			}
			...
			void drawer::_m_button_frame(graph_reference graph, rectangle r, states state)
			{
				...
				::nana::color clr{104, 104, 104}; //highlight
				switch(state)
				{
				case states::actived:
					clr.from_rgb(158, 158, 158); break;
				case states::selected:
					clr.from_rgb(200, 200, 200); break;
				default: break;
				}
